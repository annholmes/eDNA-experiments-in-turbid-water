...
##  This is the start of cmerkes github repository via Klymus et al. 2019
...
setwd("/Users/annholmes/github/ann_holmes/DSM_eDNA")

## Read in your data file (MODIFY FILE NAME AS NEEDED):
DAT <- read.csv("DSM_eDNA_LOD_LOQ.csv")
...
write("rep6.LOD: The effective limit of detection if analyzing in 6 replicates.",file="Analysis Log.txt",append=TRUE)
write("rep7.LOD: The effective limit of detection if analyzing in 7 replicates.",file="Analysis Log.txt",append=TRUE)
...
for(i in 1:length(Targets)) {
  if(!is.na(LOD.list2[i+1])) {
    DAT4 <- rbind(ED(get(LOD.list2[i+1]),0.95,interval="delta",type="absolute"),
                  ...
                  ED(get(LOD.list2[i+1]),1-0.05^(1/6),interval="delta",type="absolute"),
                  ED(get(LOD.list2[i+1]),1-0.05^(1/7),interval="delta",type="absolute"),
    ...
    DAT4 <- data.frame(DAT4,LoD=c("1rep.LOD","2rep.LOD","3rep.LOD","4rep.LOD",
                                  "5rep.LOD","6rep.LOD","7rep.LOD","8rep.LOD"),
    ...
    COLS <- c(rgb(0.8,0.47,0.65),rgb(0,0.45,0.7),rgb(0.94,0.89,0.26),
              ...
              rgb(0.43,0.21,0.1),rgb(0.23,0.27,0.29))
    PNTS <- c(15,16,17,18,25,3)
    YS <- c(0.95,1-sqrt(0.05),1-0.05^(1/3),1-0.05^0.25,1-0.05^0.2,1-0.05^(1/6),1-0.05^(1/7),1-0.05^0.125)
    LODS2 <- c("Limit of Detection","2 Replicates LoD","3 Replicates LoD",
               "4 Replicates LoD","5 Replicates LoD","6 Replicates LoD",
               "7 Replicates LoD","8 Replicates LoD")
    if(LODS<8) {
      LODS2[(LODS+1):8] <- gsub("licates LoD","s: Insufficient Data",LODS2[(LODS+1):8])
    }
